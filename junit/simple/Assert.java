...
public class Assert {
    ...
    public static void assertArrayEquals(String message, double[] expecteds,
        ...
        int exlen = expecteds.length;
        int aclen = actuals.length;
        if (exlen != aclen) {
            throw new AssertionError((message == null ? "Test failure: " : message) + " - array lengths not equal. Expected: " + exlen + " != " + aclen);
        }
        for (int i = 0; i < exlen; i++) {
            double exval = expecteds[i];
            double acval = actuals[i];
            if (exval != acval && Math.abs(exval - acval) > delta)
                throw new AssertionError((message == null ? "Test failure: " : message) + " - array element comparison failed: [" + i + "] - expected " + exval + " != " + acval + " within: " + delta + " range");
        }
    }
    ...
    public static void assertArrayEquals(String message, float[] expecteds,
        ...
        for (int i = 0; i < exlen; i++) {
            float exval = expecteds[i];
            float acval = actuals[i];
            if (exval != acval && Math.abs(exval - acval) > delta)
                throw new AssertionError((message == null ? "Test failure: " : message) + " - array element comparison failed: [" + i + "] - expected " + exval + " != " + acval + " within: " + delta + " range");
        }
    }
    ...
    private static void internalArrayEquals(String message, Object expecteds,
            Object actuals) {
        int exlen = DynUtil.getArrayLength(expecteds);
        int aclen = DynUtil.getArrayLength(actuals);
        if (exlen != aclen) {
            throw new AssertionError((message == null ? "Test failure: " : message) + " - array lengths not equal: " + exlen + " != " + aclen);
        }
        for (int i = 0; i < exlen; i++) {
            Object exval = DynUtil.getArrayElement(expecteds, i);
            Object acval = DynUtil.getArrayElement(actuals, i);
            if (exval != acval && (exval == null || !exval.equals(acval)))
                throw new AssertionError((message == null ? "Test failure: " : message) + " - array element comparison failed: [" + i + "] - expected " + exval + " != " + acval);
        }
    }
